...
function Login() {
  ...
  try {
    const response = await axios.post('http://localhost:5000/serviceofficelogin', {
    ...
    if (response.data.success) {
      ...
      setTimeout(() => {
        if (response.data.user && response.data.user.id) {
          window.location.href = `/serviceofficedashboard?branch_id=${response.data.user.branch_id}`;
        } else {
          window.location.href = '/serviceofficedashboard';
        }
      }, 1500);
    } else {
